...
class CalContentProvider(ContentProvider):
  ...
  def get_max_entries(self):
    ...
    return int((self.screen._opts.HEIGHT-
                self.screen._opts.HEIGHT_S-self.screen._y_off)/
                                                    self.screen._opts.HEIGHT_E)
  ...
  def _get_agenda(self):
    ...
    entries = []
    for cal_info in self.screen._opts.cals:
  ...
  def _draw_entry(self,e_time,e_text,e_color):
    ...
    background = [(0,self.screen._y_off+1),
                    (self.screen._opts.WIDTH,
                                    self.screen._y_off+self.screen._opts.HEIGHT_E-1)]
    self.screen._canvas.rectangle(background,fill=e_color)
  
    # time-value
    tm      = e_time.split('-')
    tm_size = [0,0]
    tm_size[0] = self.screen._canvas.textsize(tm[0],self.screen._time_font,spacing=0)
    tm_size[1] = self.screen._canvas.textsize(tm[1],self.screen._time_font,spacing=0)
    if e_time != "00:00-23:59":
      # only print time for none full-day events
      self.screen._canvas.text((self.screen._opts.MARGINS[2],self.screen._y_off+2),
                        tm[0],font=self.screen._time_font,
                        fill=self.screen._bg_map[e_color])
      self.screen._canvas.text((self.screen._opts.MARGINS[2],self.screen._y_off+4+tm_size[0][1]),
                        tm[1],font=self.screen._time_font,
                        fill=self.screen._bg_map[e_color])

    # text (2 lines)
    txt_x_off = self.screen._opts.MARGINS[2] + max(tm_size[0][0],tm_size[1][0]) + 4
    txt_y_off = self.screen._y_off + 2
    text_size = self.screen._canvas.textsize(e_text[0],
                                      self.screen._text_font,spacing=0)
    self.screen._canvas.text((txt_x_off,txt_y_off),e_text[0],
                      font=self.screen._text_font,
                      fill=self.screen._bg_map[e_color])

    txt_y_off += text_size[1]
    self.screen._canvas.text((txt_x_off,txt_y_off),
                      e_text[1],font=self.screen._text_font,
                      fill=self.screen._bg_map[e_color])

    # ending line
    self.screen._y_off += self.screen._opts.HEIGHT_E
